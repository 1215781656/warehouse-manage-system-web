        ...
        <el-row :gutter="24">
          <el-col :span="12">
            <el-form-item label="出货日期">
              <el-date-picker v-model="form.outboundDate" style="width: 100%" />
            </el-form-item>
            <!-- <el-form-item label="货单号">
              <el-input v-model="form.outboundNo" />
            </el-form-item> -->
...
const unpaidAmount = computed(() => {
  const r = Number(form.value.receivableAmount || 0)
  const g = Number(form.value.receivedAmount || 0)
...
const save = async () => {
  try {
    saving.value = true
    const resp = await financeAPI.createReceivable({
      outboundDate: form.value.outboundDate,
      outboundNo: form.value.outboundNo,
      customer: form.value.customer,
      deliveryNo: form.value.deliveryNo,
      ...
      receivedAmount: form.value.receivedAmount,
    ...
    // Wait a bit for uploads to start/finish
